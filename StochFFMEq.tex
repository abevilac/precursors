...
Finally, we fit the stochastic parameters $\sigma$ and $\gamma$ on the residuals of this linearized problem, imposing the constant $K=\frac{\sigma^2}{2\gamma}$ to be equal to their variance. In summary, we plug-in $\alpha$ from classical LLT or HT, and then we obtain $\left(A,\hat\eta(t_0),K\right)$ as required to the SDE definition. The numerical solution of the SDE is performed by the Euler-Maruyama method, which is equivalent to the Milstein method in our case.
\vskip.2cm
