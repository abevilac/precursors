...
Several methods have been adopted in the determination of the parameters in the ODE problem \citep{Voight1988, Cornelius1995}. The Log-rate versus Log-acceleration Technique (LLT), and the Hindsight Technique (HT) can both provide estimates of $\alpha$. They are described in Appendix \ref{A-1}. The first technique is generally less accurate because needs an estimate of the time derivative of the observations. The second technique requires that we know $t_e$ and hence can only be used in retrospective analysis. We take advantage of these classical methods to estimate $\alpha$ in our examples, approximating the curvature of the mean solution from the available observations.
...
Finally, we fit the stochastic parameters $\sigma$ and $\gamma$ on the residuals of this linearized problem, imposing the constant $K=\frac{\sigma^2}{2\gamma}$ to be equal to their variance. In summary, we plug-in $\alpha$ from classical LLT or HT, and then we obtain $\left(A,\hat\eta(t_0),K\right)$ as required to the SDE definition.
\vskip.3cm
...
\begin{description}
  \item[Mt. St. Helens, 1982 - line length] (a) If $t_1=t_e-20$, in Method 1 $E[t_f]$ overestimates $t_e$ of $90$ days, in Method 2 of $40$, in Method 3 of $80$. Uncertainty is $[-89,+384]$ in Method 1, $[-18,+17]$ in Method 2, and $[-82,+301]$ in Method 3. Only in Method 2 $E[t_f]$ falls outside the uncertainty range, and $95^{th}$ percentile plot in Method 3 is about 6\% at time $t_e$. In Methods 1 and 3, $P\{t_f=\infty\} > 15\%$.
      \vskip.1cm
      (b) If $t_1=t_e-15$, in Method 1 $E[t_f]$ overestimates $t_e$ of $37$ days, in Method 2 of $18$, in Method 3 of $30$. Uncertainty is $[-39,+125]$ in Method 1, $\pm 12$ in Method 2, and $[-35,+86]$ in Method 3. Again only in Method 2 $E[t_f]$ falls outside the uncertainty range, and $95^{th}$ percentile plot in Method 3 is about 8\% at time $t_e$. In Methods 1 and 3, $P\{t_f=\infty\} \approx 2\%$.
      \vskip.1cm
      (c) If $t_1=t_e-10$, in Method 1 $E[t_f]$ estimates correctly $t_e$, with an uncertainty range of $[-6,+11]$ days. In Method 2 $E[t_f]$ underestimates $t_e$ of $1$, with an uncertainty range of $\pm 3$. Method 3 performs similarly to Method 1, and its $95^{th}$ percentile plot is about 16\% at time $t_e$.

  \item[Bezymyanny, 1960 - seismic strain] (d) If $t_1=t_e-17$, in Method 1 $E[t_f]$ overestimates $t_e$ of $21$ days, in Method 2 of $4$, in Method 3 of $12$. Uncertainty is $[-32,+135]$ in Method 1, $[-9,+10]$ in Method 2, and $[-20,+86]$ in Method 3. In all methods $E[t_f]$ falls inside the uncertainty range, and $95^{th}$ percentile plot in Method 3 is about 7.5\% at time $t_e$. In Methods 1 and 3, $P\{t_f=\infty\} \approx 9\%$.
      \vskip.1cm
      (e) If $t_1=t_e-10$, in Method 1 $E[t_f]$ overestimates $t_e$ of $3$ days, in Method 2 estimates $t_e$ correctly, in Method 3 overestimates $t_e$ of $2$ days. Uncertainty is $[-10,+18]$ in Method 1, $[-6,+7]$ in Method 2, and $[-10,+17]$ in Method 3. The $95^{th}$ percentile plot in Method 3 is about 10\% at time $t_e$.
      \vskip.1cm
      (f) If $t_1=t_e-5$, in Method 1 $E[t_f]$ underestimates $t_e$ of $1$ day, in Method 2 of $2$. Method 3 performs similarly to Method 1. Uncertainty is $[-3,+6]$ in Method 1, $[-2,+3]$ in Method 2, and $[-3,+7]$ in Method 3. The $95^{th}$ percentile plot in Method 3 is about 16\% at time $t_e$. We remark that in Methods 1 and 3, $P\{t_f<t_1\} \approx 15\%$.
\end{description}
