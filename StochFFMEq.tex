...
for a.e. $\omega\in\Omega$, where $(\mathcal F_t)_{t>t_0}$ is the filtration generated by the noise, and $P$ is a probability measure over it \citep{Karatzas1991}. Plots \ref{Fig2}b,d,f display pdf of $t_f$ calculated by Monte Carlo simulation (2,000 samples). The pdf becomes peaked and symmetric when $\gamma>0$.
...
In general, the mean path is consistent in the three methods, but UQ is significantly different, as well as the values of $g_{t_f}$. In particular: \begin{description}
  \item[(a) Mt. St. Helens, 1982 - line length] data values are initially scattered, until $t = t_e - 20$, and then become more aligned. $\alpha \approx 2$, and $E[t_f]$ overestimates $t_e$ of $1$-$3$ days in all the methods. Uncertainty range is twice larger in Method 2 and 3 compared to Method 1.
  \item[(b) Mt. St. Helens, 1981 - fault movement] this example is characterized by $\alpha \approx 1.6$ in HT and $\alpha = 1.3\pm 0.2$ in LLT. In the first case (red), in all methods $E[t_f]$ underestimates $t_e$ of only $1$ day, with uncertainty range $\pm 2$ days in Method 1, and twice larger in Method 2 and 3. The second case case (blue) is less accurate. In Method 1, 2 and, 3 $E[t_f]$ overestimates $t_e$ of $14$, $11$, and $14$ days, respectively. Always outside the uncertainty range. However, in Method 3 the $95^{th}$ percentile plot is above 9\% at time $t_e$.
\end{description}
...
\begin{description}
  \item[(a) Mt. St. Helens, 1982 - line length] On the first time window (blue) $t_e-[40,20]$, in Method 1 $E[t_f]$ overestimates $t_e$ of $90$ days, in Method 2 of $40$ days, in Method 3 of $80$ days. Uncertainty is $[-89,+384]$ in Method 1, $[-18,+17]$ in Method 2, and $[-82,+301]$ in Method 3. Only in Method 2 $E[t_f]$ falls outside the uncertainty range, and $95^{th}$ percentile plot in Method 3 is about 6\% at time $t_e$. In Methods 1 and 3, $P\{t_f=\infty\} > 15\%$.
      \vskip.1cm
      On the second time window (light blue) $t_e-[40,15]$, in Method 1 $E[t_f]$ overestimates $t_e$ of $37$ days, in Method 2 of $18$ days, again in Method 3 of $30$ days. Uncertainty is $[-39,+125]$ in Method 1, $\pm 12$ in Method 2, and $[-35,+86]$ in Method 3. Again only in Method 2 $E[t_f]$ falls outside the uncertainty range, and $95^{th}$ percentile plot in Method 3 is about 8\% at time $t_e$. In Methods 1 and 3, $P\{t_f=\infty\} \approx 2\%$.
      \vskip.1cm
      On the third time window (red) $t_e-[30,10]$, in Method 1 $E[t_f]$ estimates correctly $t_e$, with an uncertainty range of $[-6,+11]$ days. In Method 2 $E[t_f]$ underestimates $t_e$ of $1$ day, with an uncertainty range of $\pm 3$ days. Method 3 performs similarly to Method 1, and its $95^{th}$ percentile plot is about 16\% at time $t_e$.
      
  \item[(b) Bezymyanny, 1960 - seismic strain] On the first time window (blue) $t_e-[37,17]$, in Method 1 $E[t_f]$ overestimates $t_e$ of $21$ days, in Method 2 of $4$ day, in Method 3 of $12$ days. Uncertainty is $[-32,+135]$ in Method 1, $[-9,+10]$ in Method 2, and $[-20,+86]$ in Method 3. In all methods $E[t_f]$ falls inside the uncertainty range, and $95^{th}$ percentile plot in Method 3 is about 7.5\% at time $t_e$. In Methods 1 and 3, $P\{t_f=\infty\} \approx 9\%$.
      \vskip.1cm
      On the second time window (light blue) $t_e-[30,10]$, in Method 1 $E[t_f]$ overestimates $t_e$ of $3$ days, in Method 2 estimates $t_e$ correctly, in Method 3 overestimates $t_e$ of $2$ days. Uncertainty is $[-10,+18]$ in Method 1, $[-6,+7]$ in Method 2, and $[-10,+17]$ in Method 3. The $95^{th}$ percentile plot in Method 3 is about 10\% at time $t_e$.
      \vskip.1cm
      On the third time window (violet) $t_e-[20,5]$, in Method 1 $E[t_f]$ underestimates $t_e$ of $1$ day, in Method 2 of $2$ days. Method 3 performs similarly to Method 1. Uncertainty is $[-3,+6]$ in Method 1, $[-2,+3]$ in Method 2, and $[-3,+7]$ in Method 3. The $95^{th}$ percentile plot in Method 3 is about 16\% at time $t_e$. We remark that in Methods 1 and 3, $P\{t_f<t_1\} \approx 15\%$.
\end{description}
...
In summary, forecasting results of Method 1 and Method 3 are similar, but the more complex UQ related to Method 3 improves its performance. In particular, when the forecast is not well constrained, Method 3 generally reduces the uncertainty range of the estimates. Method 2 tends to give a competitive forecast only when the eruption is close. The doubly stochastic formulation of Method 3 appears to have a significant impact, and the $95^{th}$ percentile of the eruption probability is significantly high at time $t_e$.
